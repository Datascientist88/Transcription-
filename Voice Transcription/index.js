...
function stop() {
    ...
    // Send the audio file to the Flask backend and display the transcription and response
    ...
    .then(data => {
        ...
        msg_box.innerHTML += '<p>Transcription: ' + transcript + '</p>';
        return fetch('/response?prompt=' + encodeURIComponent(transcript));
    })
    .then(response => response.json())
    .then(data => {
        var response = data.response;
        document.getElementById('response').innerHTML = '<p>Response: ' + response + '</p>';
    });
}
